...
// +kubebuilder:rbac:groups=extendeddeployment.io,resources=inplacesets/finalizers,verbs=update

// Reconcile is part of the main kubernetes reconciliation loop which aims to
// move the current state of the cluster closer to the desired state.
// TODO(user): Modify the Reconcile function to compare the state specified by
// the InplaceSet object against the actual cluster state, and then
...
func (r *InplaceSetReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		For(&extendeddeploymentiov1beta1.InplaceSet{}).
		Named("inplaceset").
		Complete(r)
}
